...
$mensaje='';
if($usuarioId==264){
    $query="SELECT OP.OrdenPedidoId AS OrdenPedidoId, OP.Folio AS Folio, PV.PuntoVenta AS PuntoVenta, CONCAT(E.Nombre, ' ' , E.Paterno, ' ', E.Materno) AS Usuario, OP.Estatus AS Estatus, OP.Fecha AS Fecha, OP.FechaRevision AS FechaRevision, OP.Plataforma AS Plataforma FROM OrdenPedido AS OP INNER JOIN PuntosVenta AS PV ON PV.PuntoVentaId=OP.PuntoVentaId INNER JOIN Usuarios AS U ON U.UsuarioId=OP.UsuarioId INNER JOIN Empleados AS E ON U.EmpleadoId=E.EmpleadoId INNER JOIN HistorialPuntosEmpleados AS HPE ON HPE.PuntoVentaId=OP.PuntoVentaId WHERE OP.Activo=1 AND HPE.EmpleadoId=1054";
}else{
        $query="SELECT OP.OrdenPedidoId AS OrdenPedidoId, OP.Folio AS Folio, PV.PuntoVenta AS PuntoVenta, CONCAT(E.Nombre, ' ' , E.Paterno, ' ', E.Materno) AS Usuario, OP.Estatus AS Estatus, OP.Fecha AS Fecha, OP.FechaRevision AS FechaRevision, OP.Plataforma AS Plataforma FROM OrdenPedido AS OP INNER JOIN PuntosVenta AS PV ON PV.PuntoVentaId=OP.PuntoVentaId INNER JOIN Usuarios AS U ON U.UsuarioId=OP.UsuarioId INNER JOIN Empleados AS E ON U.EmpleadoId=E.EmpleadoId WHERE OP.Activo=1 AND (OP.PuntoVentaId=$puntoVenta OR U.UsuarioId=$usuario)";
}
