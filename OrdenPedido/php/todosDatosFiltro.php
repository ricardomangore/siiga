...
if($filtro=="--Selecciona una opcion--"){
	$filtro="";
}elseif($filtro=="Estatus"){
	$filtro=" ORDER BY OP.Estatus";
}elseif($filtro=="Responsable"){
	$filtro="ORDER BY OP.Responsable";
}
...
$query="SELECT OP.OrdenPedidoId AS OrdenPedidoId, OP.Folio AS Folio, OP.Semana AS Semana, PV.PuntoVenta AS PuntoVenta, PV.PuntoVentaId AS PuntoVentaId, CONCAT(E.Nombre, ' ' , E.Paterno, ' ', E.Materno) AS Usuario, OP.Estatus AS Estatus, OP.Fecha AS Fecha, OP.Plataforma AS Plataforma FROM OrdenPedido AS OP INNER JOIN PuntosVenta AS PV ON PV.PuntoVentaId=OP.PuntoVentaId INNER JOIN Usuarios AS U ON U.UsuarioId=OP.UsuarioId INNER JOIN Empleados AS E ON U.EmpleadoId=E.EmpleadoId WHERE OP.Semana=$numeroSemana AND OP.Activo=1 AND OP.Pendiente=0 $filtro";
