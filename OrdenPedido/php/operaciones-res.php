<?php 
	...
	function inventarioActual($puntoVentaId){
		$queryInventario="SELECT DISTINCT I.Serie FROM Inventario AS I INNER JOIN Recepciones AS R ON I.MovimientoId=R.MovimientoId WHERE I.Cantidad>0 AND R.PuntoVentaId=$puntoVentaId";
		if($resultado=mysql_query($queryInventario)){
			$totalInventario=mysql_num_rows($resultado);
			$queryInvTR="SELECT DISTINCT I.Serie FROM Inventario AS I INNER JOIN TRSalidas AS TR ON I.MovimientoId=TR.MovimientoId WHERE I.Cantidad>0 AND TR.PuntoVentaIdD=$puntoVentaId AND TR.EstatusTraspasoId=1";
			$resTR=mysql_query($queryInvTR);
			$totalTR=mysql_num_rows($resTR);
			$queryInvOC="SELECT DISTINCT Serie FROM OrdenesCompra WHERE Recibido=0 AND PuntoVentaId=$puntoVentaId";
	...
	function precioTotal($puntoVentaId){
		...
		if($resultado=mysql_query($queryInventario)){
			...
			$queryInvTR="SELECT DISTINCT OC.Costo AS CostoEquipo, I.Serie AS Serie FROM Inventario AS I INNER JOIN TRSalidas AS TR ON I.MovimientoId=TR.MovimientoId INNER JOIN  Equipos AS E ON E.EquipoId=I.EquipoId  INNER JOIN OrdenesCompra AS OC ON OC.Serie=I.Serie WHERE I.Cantidad>0 AND TR.PuntoVentaIdD=$puntoVentaId AND TR.EstatusTraspasoId=1";
			if($resultadoTR=mysql_query($queryInvTR)){
				while($rowPrecioTR=mysql_fetch_array($resultadoTR)){
					$costo=$costo+$rowPrecioTR["CostoEquipo"];
				}
				$queryInvOC="SELECT OC.Costo AS CostoEquipo FROM OrdenesCompra AS OC INNER JOIN Equipos AS E ON E.EquipoId=OC.EquipoId WHERE OC.Recibido=0 AND OC.PuntoVentaId=$puntoVentaId";
