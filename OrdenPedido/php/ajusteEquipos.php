<?php 
	...
	while($rowEquipos=mysql_fetch_array($resultado)){
		...
		//	echo $equipos2;
			...
			if($cont>0){
				$query="UPDATE OrdenPedido SET EquiposAux='$equipos2' WHERE OrdenPedidoId='$ordenPedido'";
				if($resQuery=mysql_query($query)){
					echo '
					<div class="alert alert-success alert-dismissable" align="center">
  <button type="button" class="close" data-dismiss="alert">&times;</button>
  <b><span class="glyphicon glyphicon-ok"></span> Correcto</b> Informaci&oacute;n Actualizada con exito.
</div>';
					/*respuesta de ajuste */
					$mensaje='';
$time=time();
...
$query="SELECT * FROM OrdenPedido WHERE OrdenPedidoId=$ordenPedido";
//echo $query;
...
//while($resultados = mysqli_fetch_array($consulta)) {
		...
		$equipos=explode(",",$resultados["EquiposAux"]);
		$tam=count($equipos);
		...
		for($i=0;$i<($tam-1);$i++){
			...
			$queryEquipos="SELECT M.Marca AS Marca, E.Equipo AS Equipo, E.NombreConsigna AS NombreConsigna FROM Equipos AS E INNER JOIN Marcas AS M ON M.MarcaId=E.MarcaId WHERE E.EquipoId=$prueba[0]";
			...
			$nombreConsigna=$resEquipos["NombreConsigna"];
			$cantidad=$prueba[1];
...
echo '  </table>
	  ...
	  echo "<br>No pudo ejecutarse satisfactoriamente la consulta () " .$query.
...
';
					...
					echo("Error al Actualizar : ".$query.mysql_error());
				}
