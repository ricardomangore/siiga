	...
	if($resEquipos=mysql_query($queryEquipos)){
		while($rowEquipos=mysql_fetch_array($resEquipos)){
			if($_POST[$rowEquipos["EquipoId"]]>0){
				...
				if(mysql_query("INSERT INTO EquiposPedido(EquipoPedidoId, OrdenPedido, EquipoId, Cantidad, TipoEquipo, Activo) VALUES (NULL, '$ordenPedido',$equipoId,$cantidad,2,1)")){
					$querySelect="SELECT M.Marca AS Marca, E.Equipo AS Equipo,  E.EquipoId AS EquipoId, EP.Cantidad FROM EquiposPedido AS EP INNER JOIN Equipos AS E ON EP.EquipoId=E.EquipoId INNER JOIN Marcas AS M ON M.MarcaId=E.MarcaId WHERE EP.OrdenPedido='$ordenPedido' AND EP.TipoEquipo=2 AND EP.Activo=1 ORDER BY M.Marca, E.Equipo";
