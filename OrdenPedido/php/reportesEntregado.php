<?php
	...
	header("Content-Disposition: attachment; filename=reporteEquiposEntregados.xls");
	...
	<table border="1">
	<tr align="center">
		<th colspan="7" style="background: #000000; color: #FFFFFF;"><font face="helvetica">  Equipos Entregados <?php echo $semana ?> </font></th>
	</tr>
	<tr>
		<td>Semana</td>
	...
	<?php
	$query="SELECT PuntoVentaId, EquiposFinal, PuntoVentaId, Semana FROM OrdenPedido WHERE (Semana>4 AND Semana<8) AND Finalizado=1 AND Activo=1 AND Estatus!='Cancelado' ORDER BY Semana";
	//echo $query;
	if($resEquipos=mysql_query($query)){
		while($row=mysql_fetch_array($resEquipos)){
			$semanaAux=$row["Semana"];
			$equipos=explode(",",$row["EquiposFinal"]);
