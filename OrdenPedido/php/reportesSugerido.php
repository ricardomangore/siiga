<?php
	...
	if($filtros==''){
		$filtros="Estatus!='Cancelado' AND Estatus!='Nuevo' AND Activo=1";
	}else{
		$filtros=$filtros." AND Estatus!='Cancelado' AND Estatus!='Nuevo' AND Activo=1";
	}
	?>
	<table border="1">
	<tr>
	...
	<?php
	$query="SELECT PuntoVentaId, Equipos, PuntoVentaId, FechaRevision FROM OrdenPedido WHERE $filtros";
	//echo $query;
	if($resEquipos=mysql_query($query)){
		while($row=mysql_fetch_array($resEquipos)){
		    $fecha=$row["FechaRevision"];
