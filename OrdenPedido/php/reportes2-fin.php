<?php
	...
	if($bandFecha==1){
		if($fechaInicial<=$fechaFin){
			...
			$condiciones=" AND (Fecha>'$fechaInicial' AND Fecha<'$fechaFin')";
			$condiciones2=" AND (OP.Fecha>'$fechaInicial' AND OP.Fecha<'$fechaFin')";
		}else{
	...
	if($res=mysql_query($query)){
		...
		?>
			...
			<?php
					$queryPunto="SELECT OP.Plataforma AS Plataforma, OP.Semana AS Semana, PA.SubInventario AS SubInventario, PA.NombreATT AS NombreATT, OP.Equipos AS Equipos FROM OrdenPedido AS OP INNER JOIN PuntosATT AS PA ON OP.PuntoVentaId=PA.PuntoVentaId WHERE OP.Activo=1 $condiciones2 ORDER BY PA.SubInventario ASC";
