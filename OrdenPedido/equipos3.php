<?php
	...
	$querySemana="SELECT Semana FROM OrdenPedido WHERE (Semana=$semanaFin) AND PuntoVentaId=$puntoVentaId AND Finalizado=0 AND Estatus!='Cancelado'";
	/*if($resSemana=mysql_query($querySemana)){
		if(mysql_num_rows($resSemana)>0){
			echo "
				<script>
...
window.location.href='ordenPedido.php';
		...
		$queryUnico="SELECT Semana FROM OrdenPedido WHERE Semana=$numeroSemana AND PuntoVentaId=$puntoVentaId AND Activo=1";
	if((mysql_num_rows(mysql_query($queryUnico)))==1){
		echo "
				<script>
alert('Ya se ha cargado la orden de la semana ".$numeroSemana."');
window.location.href='ordenPedido.php';
</script>";
	...
	if(mysql_num_rows(mysql_query($res))>0){
		...
		if($puntoVentaId==427 || $puntoVentaId==426  || $puntoVentaId==471  || $puntoVentaId==509  || $puntoVentaId==494  || $puntoVentaId==496  || $puntoVentaId==34  || $puntoVentaId==368  || $puntoVentaId==493  || $puntoVentaId==42  || $puntoVentaId==21  || $puntoVentaId==29  || $puntoVentaId==369  || $puntoVentaId==370  || $puntoVentaId==371  || $puntoVentaId==76  || $puntoVentaId==30  || $puntoVentaId==72  || $puntoVentaId==407  || $puntoVentaId==425  || $puntoVentaId==74  || $puntoVentaId==492   || $puntoVentaId==506   || $puntoVentaId==67   || $puntoVentaId==467   || $puntoVentaId==464   || $puntoVentaId==61   || $puntoVentaId==68   || $puntoVentaId==397   || $puntoVentaId==9   || $puntoVentaId==48   || $puntoVentaId==457 || $puntoVentaId==75 || $puntoVentaId==491){
			$responsable="Elizabeth Sanchez Huitzil";
		}else{
			$responsable='Jaime Rodriguez Sanchez';
		}
		if(mysql_query("INSERT INTO OrdenPedido(OrdenPedidoId, PuntoVentaId, Plataforma, Equipos, Fecha, Semana, UsuarioId, Estatus, Activo, Pendiente, Responsable) VALUES (NULL, $puntoVentaId,'Pendiente', 'Pendiente', '2000-01-01',0,$usuarioId,'Pendiente', 0,1,'$responsable')")){
...
function buscar() {
  ...
  <li class="active"><span class="circulo">1</span> Selecciona Equipos</li>
   ...
   <div align="center">
		 ...
		 $pagina->agregarConsulta("SELECT E.MarcaId AS MarcaId, E.EquipoId AS EquipoId, E.Equipo AS Equipo, E.NombreConsigna AS NombreConsigna, E.CostoEquipo AS CostoEquipo FROM Equipos AS E INNER JOIN Marcas AS M ON E.MarcaId=M.MarcaId WHERE M.Activo=1 AND E.Activo=1 AND E.OrdenPedido=1 ORDER BY M.Marca");
		 $pagina->ejecutar();
		$cont=0;
