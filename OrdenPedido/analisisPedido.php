  ...
  <head>
    ...
    <?php
			if($rowEquipos["Estatus"]=="Finalizado"){
				echo 'url: "php/respuestasPedido.php?id=+'.$ordenPedidoId.'",';
			}else{
				echo 'url: "php/respuestas2.php?id=+'.$ordenPedidoId.'",';
			}
		?>
    type: "POST",
    dataType: "html",
    ...
    var textoBusqueda = $("input#busqueda").val();
     ...
     if (textoBusqueda != "") {
        $.post("php/buscarEquipos.php", {valorBusqueda: textoBusqueda}, function(mensaje) {
            $("#resultadoBuscarEquipos").html(mensaje);
         }); 
     }
};			
...
$(function(){
 ...
 var url = "php/agregarEquipoAdmin.php"; // El script a dónde se realizará la petición.
    $.ajax({
           type: "POST",
           url: url,
  ...
  <div class="modal fade" id="myModal" role="dialog">
    <div class="modal-dialog">
      ...
      <!--inicio de modal-->
  <!-- Modal -->
  <div class="modal fade" id="buscadorEquipos" role="dialog">
        ...
        <div class="modal-body">
         <form id="formularioAgregarEquipo">
        	<input type="hidden" name="ordenPedido" value="<?php echo $ordenPedidoId;?>">
         	...
         	<div id="resultadoBuscarEquipos"></div>
  ...
  <div class="modal fade" id="resumen" role="dialog">
    <div class="modal-dialog" style="width: 47%; ">
      ...
      <!-- Modal content-->
      <div class="modal-content">
        <div class="modal-header" style="background: #009EDB; color: #FFFFFF;">
          <button type="button" class="close" data-dismiss="modal">&times;</button>
          <h4 class="modal-title" align="center">Resumen de Pedido:<b> <?php echo $rowEquipos["PuntoVenta"];?></b> Semana: <b> <?php echo $rowEquipos["Semana"];?></b></h4>
        </div>
        ...
        <div style="width: 70%">
         ...
         <table class="table table-striped">
         	<tr>
         		<td><b>Total de Equipos Solicitados</b></td>
         		<td><?php echo totalEquiposPedido($ordenPedidoId); ?></td>
         	</tr>
         	<tr>
         		<td><b>Total de Equipos Autorizados</b></td>
         		<td><?php echo totalEquiposAutorizado($ordenPedidoId); ?></td>
         	</tr>
         	<tr>
         		<td><b>Total de Equipos Entregados</b></td>
         		<td><?php echo totalEquiposEntregados($ordenPedidoId); ?></td>
         	</tr>
        ...
        <div class="modal-footer">
	      ...
	      <button type="button" class="btn btn-info" data-toggle="modal" data-target="#resumen"><span class="glyphicon glyphicon-duplicate"></span> Resumen de Pedido</button>
	   <?php
			if($rowEquipos["Estatus"]=="Finalizado"){
				?>
...
?>
   ...
   <div class="row">
	...
	<form id="formularioAjusteEquipos" method="post">
		...
		<div>
		<?php
			if($rowEquipos["Estatus"]=="Finalizado" || $rowEquipos["Estatus"]=="Revisado"){
				?>
				<button type="button" class="btn btn-info btn-xs" data-toggle="modal" data-target="#buscadorEquipos" disabled><span class="glyphicon glyphicon-plus"></span> Agregar Equipos</button>
				<?php
			}else{
				?>
							<button type="button" class="btn btn-info btn-xs" data-toggle="modal" data-target="#buscadorEquipos"><span class="glyphicon glyphicon-plus"></span> Agregar Equipos</button>
				<?php
			}
		...
		<?php
			if($rowEquipos["Estatus"]=="Finalizado" || $rowEquipos["Estatus"]=="Revisado"){
				?>
				<button id="ajusteEquipos" class="btn btn-primary" disabled><span class="glyphicon glyphicon-hand-right"></span>  Procesar</button>
				<input type="button" class="btn btn-danger" onClick="location.href='analisisPedido.php?id=<?php echo $ordenPedidoId; ?>'" value=" x Cancelar" disabled>
				<?php
			}else{
				?>
		...
		<form action="php/guardar.php" method="post">
			...
			<div align="center">
				<?php
					if($rowEquipos["Estatus"]=="Revisado" || $rowEquipos["Estatus"]=="Finalizado"){
						?>
						<button class="btn btn-success" name="guardar" disabled><span class="glyphicon glyphicon-floppy-saved"></span> Guardar Orden</button> 
						<?php
					}else{
						?>
						<button class="btn btn-success" name="guardar"><span class="glyphicon glyphicon-floppy-saved"></span> Guardar Orden</button> 
						<?php
					}
				?>
				<button type="button" class="btn btn-danger" data-toggle="modal" data-target="#myModal"><span class="glyphicon glyphicon-remove"></span> Cancelar Orden</button>
			</div>
